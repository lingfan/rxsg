...
<body>
  ...
  <form action="TradeFunction.php" method="POST" name="formphpda" id="formphpda" target="dformsubmit">
   <div>
   <div class="t_r"> 
      ...
      <table id="selltable1">           
        ...
        ?> 
          <tr style="background:#00FF33;color:#FF0000;margin:5px 10px;font-size:14px;">
	        ...
	        <td width="20%" class="bordertop"><? echo $arrmortype[$armorinfo['type']].'·'.$arrmortypes[$armorinfo['combine_level']];?></td>
	        <td width="20%" class="bordertop"><? echo $armorinfo['strong_level'];?></td>
	        <td width="20%" class="bordertop"><? if(empty($armorinfo['best_quality'])) echo '---';else{$atibutes=explode(",",$armorinfo['best_quality']);echo $atibutes[3].'·'.$atibutes[4].'+'.$atibutes[2].'%';};?></td>
		    <td width="20%" class="bordertop" style="text-align:right;"><? echo $armorinfo['price'].'元';?>
		      <input type="checkbox" name="cancelsell" id="<?echo $armorinfo['sid'];?>" value="<?echo $armorinfo['sid'];?>" onclick="showmysure('<?echo $suresid;?>','formphpda','iframesellaId','sellarmors.php',event);">
			</td>
           </tr>
		<? 
        } 
