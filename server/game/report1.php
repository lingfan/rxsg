...
//    if (!sql_check("select * from sys_sessions where uid='$uid' and sid='$sid' and ip='$ip'"))
     ...
     */
    $useOld=isset($_GET['useNew'])?false:true;
    if($type=="report")
    {
    	$tablename="sys_report";
		$report = sql_fetch_one("select `id`,`uid`,from_battlenet,from_unixtime(`time`,'%Y%m%d') as day from $tablename where uid='$uid' and id='".$id."'");
		if($report['from_battlenet']==0){
			$useOld=false;
		}
    }
