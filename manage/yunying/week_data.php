...
try {
	...
	switch ($type){
		case 'week_active_user':{
			...
			$n = count($table);
			for($i=0;$i<7;$i++){
				$b_log = date('Y-m-d',$day-(7-$i)*86400);
				$a = "log_user_".$b_log;
				if(in_array($a,$table)){
					break;
				}
			...
			$result = sql_fetch_one_cell("select count(f.uid) from `log_user_$a_log` as f,`log_user_$b_log` as e where f.uid = e.uid and f.onlinetime-e.onlinetime > 2.5 * 3600",'bloodwarlog');
			if(!empty($result)){
				$ret['content']['week_active_user'] = $result;
			}else {
				$ret['content']['week_active_user'] = 0;
			}
			break;
		}
		case 'week_login_num':{
			/*************登陆用户*************/
			$sql = "select count(distinct uid) from log_login where time < $day and time >=$day-7*86400";
			break;
		}
		case 'week_pay_num':{
			/************充值人数(周)************/
			$sql = "select count(distinct(passport)) from pay_log where time < '$day' and time >='$day'-7*86400";
			break;
		}
		case 'week_pay_add_num':{
			/************新增充值人数(周)************/
			$sql = "select count(distinct(passport)) from pay_log where time < '$day' and time >='$day'-7*86400 and passport not in (select passport from pay_log where time <'$day'-7*86400)";
			break;
		}
		case 'week_pay_account':{
			/***********最高充值账号(周)****************/
			$sql = "select passport from pay_log where time < $day and time >=$day-7*86400 and money = (select max(money) from pay_log)";
			break;
		}
		case 'week_pay_money':{
			/*************周充值总金额*************/
			$sql = "select sum(money) from pay_log where time < $day and time >=$day-7*86400";
			break;
		}
		case 'pay_num':{
			/*************充值次数(周)*************/
			$sql = "select count(*) from pay_log where time < $day and time >=$day-7*86400";
			break;
		}
		case 'week_build_user_loss':{
			//建号用户流失总数
			$result = sql_fetch_one_cell("select count(uid) from sys_online where ('$day'-lastupdate)/86400 >= 30");
			if(!empty($result)){
				$ret['content']['week_build_user_loss'] = $result;
			}else {
				$ret['content']['week_build_user_loss'] = 0;
			}
			break;
		}
		case 'week_old_user_lost':{
			/***********日老用户流失数*************/
			...
			if(!empty($result)){
				$ret['content']['week_old_user_lost'] = $result;
			}else {
				$ret['content']['week_old_user_lost'] = 0;
			}
			break;
		}
		case 'week_pay_new_user':{
			/***********付费新建号用户*************/
			...
			if(!empty($result)){
				$ret['content']['week_pay_new_user'] = $result;
			}else {
				$ret['content']['week_pay_new_user'] = 0;
			}
			break;
		}
		case 'week_active_pay_user':{
			//活跃用户中的充值用户
			$a_log = date('Y-m-d',$day);
			$tables = sql_fetch_rows("show tables like 'log_user_%'",'bloodwarlog');
			foreach ($tables as $value){
			...
			$act_uid = sql_fetch_rows("select f.uid from `log_user_$a_log` as f,`log_user_$b_log` as e where f.uid = e.uid and f.onlinetime-e.onlinetime > 2.5 * 3600",'bloodwarlog');
			...
			if(!empty($result)){
				$ret['content']['week_active_pay_user'] = $result;
			}else {
				$ret['content']['week_active_pay_user'] = 0;
			}
			break;
		}
		case 'week_new_user_lost':{
			/***********新建号流失数*************/
			...
			$result = $sum - $res;
			if(!empty($result)){
				$ret['content']['week_new_user_lost'] = $result;
			}else {
				$ret['content']['week_new_user_lost'] = 0;
			}
			break;
		}
	}
