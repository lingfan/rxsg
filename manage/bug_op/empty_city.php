...
if ((empty($passport)) && (empty($cid))) {
    ...
    if (! empty($city) && $city['passport'] == $passport) {
        ...
        if (empty($mem_city)) {
            ...
            sql_query("update mem_city_resource m set m.`people_max`=(select sum(level*(level+1)*50) from sys_building s where s.`cid`=m.cid and s.bid=5) where m.`cid`='$cid'");
            sql_query("update mem_city_resource m set `gold_max`=(select level*(level+1)*500000 from sys_building s where s.`cid`=m.cid and s.bid=6) where m.`cid`='$cid'");
            sql_query("update mem_city_resource set `people_stable`=`people_max`*morale*0.01 where `cid`='$cid'");
            sql_query("insert into sys_city_res_add (cid,resource_changing) values ('$cid',1) on duplicate key update resource_changing=1");
            sql_query("update mem_city_resource set people=GREATEST(people,people_stable) where cid='$cid'");
            $ret = "修复成功";
        } else {
            $ret = "该城池不需要修复！";
        }
