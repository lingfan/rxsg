...
if(!empty($from_serverid)&&empty($sql_error)){
	$sql = "select u.name as uname,u.passport,cg.name as gname,lg.count,from_unixtime(lg.time) as time,lg.type 
			...
			where lg.uid=u.uid and lg.gid=cg.gid and u.passport='$passport' and u.from_serverid='$from_serverid' and lg.time>=unix_timestamp('$startday') and lg.time<unix_timestamp('$endday')+86400 and lg.gid>=160001 and lg.gid<=160051
			order by lg.gid";
