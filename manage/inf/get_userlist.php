...
function format_time($long_time)
	...
	$ret = sql_fetch_rows("select u.nobility,u.officepos,u.uid as uid,name,passport,`group`,state,prestige,rank,union_id,money,lastupdate,lastcid,onlinetime,from_unixtime(regtime) as regtime from sys_sessions s,sys_online o,sys_user u where u.name like '%$name%' and s.uid=u.uid and s.uid=o.uid");
	$userlist = $ret;
