...
$sql_tid = sql_fetch_one_cell("select tid from sys_technic where tid='$tid' and uid='$uid'");
if (empty($sql_tid)) {
    $sql_cid = sql_fetch_one_cell("select cid from sys_technic where uid = $uid limit 1");
    sql_query("insert into sys_technic(uid,tid,level,state,cid) values($uid, $tid, $level, 0, '$sql_cid')");
} else {
